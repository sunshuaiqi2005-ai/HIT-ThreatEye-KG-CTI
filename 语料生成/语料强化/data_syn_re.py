...
def get_synonyms(word):
    """获取自定义同义词，增加随机性"""
    synonyms = custom_synonyms.get(word, [])
    return synonyms if len(synonyms) > 1 else []


def aggressive_replacement(text, min_replace=2, max_replace=5):
    """
    更激进的同义词替换，确保替换足够多的词汇
    :param min_replace: 最小替换数
    :param max_replace: 最大替换数
    """
    words = jieba.lcut(text)
    candidates = [i for i, word in enumerate(words) if word in custom_synonyms]

    if not candidates or len(candidates) < min_replace:
        return None  # 不返回替换不足的文本

    num_replace = random.randint(min_replace, min(max_replace, len(candidates)))
    selected_indices = random.sample(candidates, num_replace)
...
def augment_data(data, similarity_threshold=0.6):
    """
    严格的数据增强：
    1. 每个原始数据最多生成1个增强版本
    2. 确保增强版本与原始文本相似度<阈值
    ...
    augmented_data = []
    seen_hashes = set()
    similar_pairs = []  # 存储相似度高于阈值的文本对

    for item in data:
        # 添加原始数据
        original_hash = generate_item_hash(item)
        if original_hash not in seen_hashes:
            augmented_data.append(item.copy())
            seen_hashes.add(original_hash)

        # 生成增强版本
        original_text = f"{item.get('instruction', '')} {item.get('output', '')}"
        attempt = 0
        max_attempts = 10
        ...
        while attempt < max_attempts:
            ...
            # 对instruction和output都进行替换
            if 'instruction' in new_item:
                new_instruction = aggressive_replacement(new_item['instruction'])
                if not new_instruction:
                    continue
                new_item['instruction'] = new_instruction

            if 'output' in new_item:
                new_output = aggressive_replacement(new_item['output'])
                if not new_output:
                    continue
                new_item['output'] = new_output

            # 检查相似度
            new_text = f"{new_item.get('instruction', '')} {new_item.get('output', '')}"
            similarity = jaccard_similarity(original_text, new_text)
            ...
            if similarity >= similarity_threshold:
                ...
                similar_pairs.append({
                    ...
                    'augmented': new_text[:200] + "..." if len(new_text) > 200 else new_text,
                    'reason': f"相似度 {similarity:.2f} >= 阈值 {similarity_threshold}"
                })
                continue
    ...
    # 打印相似文本统计
    print(f"\n=== 相似文本过滤统计 ===")
    print(f"总检查文本对: {len(similar_pairs)}")
    print(
        f"平均相似度: {sum(p['similarity'] for p in similar_pairs) / len(similar_pairs):.2f}" if similar_pairs else "无相似文本")

    # 打印前5个最相似的文本对
    if similar_pairs:
        print("\n=== 前5个被过滤的高相似度文本 ===")
        for i, pair in enumerate(sorted(similar_pairs, key=lambda x: -x['similarity'])[:5]):
            print(f"\n#{i + 1} 相似度: {pair['similarity']:.2f}")
            print(f"原始文本: {pair['original']}")
            print(f"增强文本: {pair['augmented']}")
            print(f"过滤原因: {pair['reason']}")

    return augmented_data
# def augment_data(data, similarity_threshold=0.75):
#     """
#     严格的数据增强：
#     1. 每个原始数据最多生成1个增强版本
#     2. 确保增强版本与原始文本相似度<0.6
#     3. 至少替换3个词汇
#     """
#     augmented_data = []
#     seen_hashes = set()
...
#                     augmented_data.append(new_item)
#                     seen_hashes.add(new_hash)
#                     break  # 成功生成一个增强版本后即停止
#
#     return augmented_data


def save_augmented_data(input_json_path, output_json_path):
    """
    高效处理大数据集的增强保存
    """
    # 使用逐行读取处理大数据集
...
if __name__ == "__main__":
    input_json_path = 'eda.json'#'./augmented_alpaca_znen.json'#'./strict_augmented_1.json' augmented_alpaca_znen.json->3 alpaca_znen.json->4
    output_json_path = 'eda.json'#'./strict_augmented_3.json'#'./strict_augmented_2.json'

    # 使用更严格的增强（相似度阈值0.6，最少替换3个词）
    save_augmented_data(input_json_path, output_json_path)